...
test("安装与卸载只处理程序并保留 Vault 个人内容", async () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(await readFile(extensionManifest, "utf8")).version, VERSION);
...
test("v0.6.0 未修改 Skill 与旧程序副本一致时可安全认领升级", async () => {
  ...
  try {
    ...
    for (const skill of SKILLS) {
      ...
      assert.equal(marker.installed_version, VERSION);
    }
  } finally {
    await fixture.cleanup();
  }
...
test("再次安装可修复 0.6.1 已迁移但任务状态和旧 Hook 留错的安装", async () => {
  ...
  try {
    ...
    assert.equal(repairedState.version, VERSION);
